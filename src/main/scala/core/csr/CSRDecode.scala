...
class CSRDecode(implicit conf: Config) extends Module {
  ...
  io.out.rs1Val := DontCare //Not assigned in here, assigned in Decode instead
  io.out.csrReg := io.instr(31,20)
  io.out.rs1 := io.instr(19, 15)
  io.out.rd :=  io.instr(11, 7)
  io.out.imm := 0.U((conf.XLEN - 5).W) ## io.instr(19, 15)
  io.out.valid := op === Opcode.SYSTEM && VecInit(validFunct3).contains(funct3)
  io.out.useImm := VecInit(Seq(Funct3.CSRRSI, Funct3.CSRRWI, Funct3.CSRRCI).map(_.U(3.W))).contains(funct3)
